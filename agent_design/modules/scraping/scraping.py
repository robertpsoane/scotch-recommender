...
UPDATE_URL_BASE = "https://www.masterofmalt.com/new-arrivals/whisky-new-arrivals/"
...
# Update Functions
def getUpdates(label, ids, stop_n):
    df = pd.DataFrame(columns=cols)

    num = 1
    cont = True
    while cont:
        url = UPDATE_URL_BASE + str(num)
        newdf, cont = getUpdateWhiskysFromURL(url, cols, label, ids, stop_n)
        df = df.append(newdf)
        num += 1
...
def getUpdateWhiskysFromURL(url, cols, label, ids, stop_n):
    pagedf = pd.DataFrame(columns=cols)
    print(f"Getting whisky's from {url}.")
    print("Getting the following attributes:")
    pprint(cols)
    cont = True
    failure = False
    ...
    try:
        ...
        if len(whiskys) > 0:
            for whisky in whiskys:
                url = whisky.a["href"]
                whisky = updateWhiskyFromURL(url, cols, label)
                if len(whisky) == 1:
                    whisky_id = whisky["ID"].values[0]
                    if whisky_id in ids:
...
def checkScotch(page):
    breadcrumbs = page.find(id="ContentPlaceHolder1_plhdBreadcrumb_breadcrumbs")
    links = breadcrumbs.find_all('a')
    scotch = False
    for a in links:
        scotch = scotch or a["href"] == "https://www.masterofmalt.com/country/scotch-whisky/"
    return scotch 

def updateWhiskyFromURL(url, cols, wtype):
    try:
        # Get text from page
        response = get(url)
        page = BeautifulSoup(response.text, 'html.parser')
